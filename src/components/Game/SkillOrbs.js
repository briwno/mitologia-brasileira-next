...
export default function SkillOrbs({
  ...
  return (
    <div className="pointer-events-none absolute inset-0 flex items-center justify-center select-none z-[80]">
      {/* Switch orb positioned further below the card (render first so it's behind) */}
      {showSwitchOrb && (
        (() => {
          const deg = 270; // below
          const rad = (deg * Math.PI) / 180;
          const extra = isSmall ? 52 : 68; // push further down to clear modal
          ...
          return (
            <button
              ...
              className={`group pointer-events-auto absolute w-14 h-14 rounded-full border-2 backdrop-blur-sm transition-transform duration-150 flex items-center justify-center text-xl shadow-xl z-10 ${
                canClick
      ...
      {skills.slice(0, 5).map((skill, i) => {
        ...
        return (
          <button
            ...
            className={`group pointer-events-auto absolute w-14 h-14 rounded-full border-2 backdrop-blur-sm transition-transform duration-150 flex items-center justify-center text-xl shadow-xl z-20 ${
              canUse
