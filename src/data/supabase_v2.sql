...
CREATE TABLE public.matches (
  ...
  player_a_has_acted boolean NOT NULL DEFAULT false,
  player_b_has_acted boolean NOT NULL DEFAULT false,
  turn_phase text DEFAULT 'action'::text,
  turn_history jsonb DEFAULT '[]'::jsonb,
  game_state jsonb DEFAULT '{}'::jsonb,
  last_action jsonb,
  last_action_timestamp timestamp with time zone,
  CONSTRAINT matches_pkey PRIMARY KEY (id),
  CONSTRAINT matches_current_player_id_check CHECK (
    current_player_id IS NULL OR current_player_id = player_a_id OR current_player_id = player_b_id
  ),
