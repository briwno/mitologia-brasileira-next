            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <div className="text-xs text-slate-300">
                  Turnos em campo: {card.onFieldTurns || 0}
                </div>
              </div>
            </div>
            <button
                  ...
                  className={`
                    ...
                    ${
                      isUltimate && !skill.locked && !disabled
                        ? "ring-2 ring-yellow-400/60 ring-offset-1 ring-offset-slate-900"
                        : ""
                    }
                  `}
                >
...
export default function GameRoom({ params }) {
  ...
  const buildAbilityPP = (card) => {
    ...
    skills.forEach((skill, idx) => {
      if (skill || idx === 4) {
        // garante slot para a 5ª (genérica pode existir)
  ...
  // FX visuais
  const [damageFx, setDamageFx] = useState([]); // {id, side: 'player'|'opponent', amount}
  const [flashFx, setFlashFx] = useState({ player: false, opponent: false });
  const [stunFx, setStunFx] = useState({ player: false, opponent: false });
  ...
  const hasFieldBuff = useCallback(
    (card) => {
  ...
  const getAttackWithField = useCallback(
    (card) => {
      if (!card) return 0;
  ...
  const mitigateIncomingDamage = useCallback(
    (targetCard, amount) => {
      let dmg = amount;
      if (
        targetCard &&
        currentField === "pantanal" &&
        targetCard.id === "bot001"
      ) {
  ...
  const getSkills = useCallback((card) => {
    ...
    allSkills.forEach((skill, idx) => {
      ...
      if (skill) {
        ...
        const ppInfo = card.abilityPP?.[key] || {
          current: skill.ppMax || getDefaultMaxPP(skill, idx),
          max: skill.ppMax || getDefaultMaxPP(skill, idx),
        };
        mapped.push({
          key: skill.key || key,
          idxKey: key,
        ...
        const genericSkills = [
          ...
          {
            name: "Atordoar",
            description: "Atordoa o alvo por 1 turno.",
            cost: 3,
            kind: "stun",
            stun: 1,
          },
        ...
        const ppInfo = card.abilityPP?.[key] || {
          current: getDefaultMaxPP(genericSkill, idx),
          max: getDefaultMaxPP(genericSkill, idx),
        };
  ...
  const canUseSkill = useCallback(
    (side, skill) => {
      ...
      const card =
        side === "player" ? activeCards.player : activeCards.opponent;
      if (!card) return false;
      const key = skill.idxKey || "skill1";
      const slot = card.abilityPP?.[key];
      const current =
        slot?.current ??
        getDefaultMaxPP(skill, parseInt(key.replace("skill", ""), 10) - 1);
      return current > 0;
    },
    [activeCards.player, activeCards.opponent]
  );
  ...
  const spendPP = useCallback((side, idxKey) => {
    setActiveCards((prev) => {
      ...
      abilityPP[idxKey] = {
        ...slot,
        current: Math.max(0, (slot.current ?? slot.max) - 1),
      };
  ...
  const applyDamage = useCallback(
    (source, targetSide, amount) => {
      setActiveCards((prev) => {
        ...
        // FX: flash e número de dano
        setFlashFx((f) => ({ ...f, [targetSide]: true }));
        const id = Date.now() + Math.random();
        setDamageFx((arr) => [
          ...arr,
          { id, side: targetSide, amount: adjusted },
        ]);
        setTimeout(() => {
          setFlashFx((f) => ({ ...f, [targetSide]: false }));
          setDamageFx((arr) => arr.filter((x) => x.id !== id));
        }, 700);
        if (newHealth <= 0) {
          ...
          pushLog({
            ...
            type: "damage",
            title: `K.O. em ${targetCard.nome || targetCard.name}`,
            desc: `Dano ${adjusted}`,
          });
  ...
  const applyPassivesOnFieldChange = useCallback(() => {
    ...
    if (currentField === "rio") {
      setActiveCards((prev) => {
        const upd = { ...prev };
        if (upd.player && upd.player.id === "iar001")
          upd.player = { ...upd.player, vida: (upd.player.vida || 0) + 1 };
        if (upd.opponent && upd.opponent.id === "iar001")
          upd.opponent = {
            ...upd.opponent,
            vida: (upd.opponent.vida || 0) + 1,
          };
        return upd;
      });
      setPlayerBench((prev) =>
        prev.map((c) =>
          c && c.id === "iar001" ? { ...c, vida: (c.vida || 0) + 1 } : c
        )
      );
      setOpponentBench((prev) =>
        prev.map((c) =>
      ...
      setTimeout(() => {
        setGameState((prev) => {
          ...
          if (prev.opponentStun > 0) {
            // FX: stun no jogador (já estava setado anteriormente se aplicou)
        ...
        setActiveCards((prev) => {
          ...
          if (ac.opponent && ac.player) {
            ...
            let chosen =
              usable.find((s) => s.key === "skill5") ||
              usable.find((s) => s.kind === "stun") ||
            ...
            if (chosen) {
              ...
              if (chosen.kind === "stun") {
                ...
                setGameState((g) => ({
                  ...g,
                  playerStun: (g.playerStun ?? 0) + (chosen.stun ?? 1),
                }));
                pushLog({
                  side: "opponent",
                  type: "stun",
                  title: `${chosen.name}`,
                  desc: `Você fica atordoado por ${chosen.stun ?? 1}T`,
                });
                setMascotMood("worried");
                // FX: stun no jogador
                setStunFx((f) => ({ ...f, player: true }));
                setTimeout(
                  () => setStunFx((f) => ({ ...f, player: false })),
                  900
                );
              } else {
                // dano
                ...
                const newHealth =
                  (ac.player.vida || 0) -
                  mitigateIncomingDamage(ac.player, dmg);
                if (newHealth <= 0) {
                  // KO do jogador: envia para mão com isDead
                  ...
                  pushLog({
                    side: "opponent",
                    type: "damage",
                    title: `K.O. em ${ac.player.nome || ac.player.name}`,
                    desc: `Dano ${dmg}`,
                  });
                  setMascotMood("worried");
                  ac.player = null;
                ...
                // FX: flash e número no jogador
                setFlashFx((f) => ({ ...f, player: true }));
                const id = Date.now() + Math.random();
                setDamageFx((arr) => [
                  ...arr,
                  { id, side: "player", amount: dmg },
                ]);
                setTimeout(() => {
                  setFlashFx((f) => ({ ...f, player: false }));
                  setDamageFx((arr) => arr.filter((x) => x.id !== id));
                }, 700);
              }
            }
        ...
        setTimeout(() => {
          ...
          // Checa mudança de campo para o próximo turno se necessário
  ...
  const castSkill = useCallback(
    (index) => {
      ...
      if (skill.kind === "stun") {
        ...
        setGameState((prev) => ({
          ...prev,
          opponentStun: (prev.opponentStun ?? 0) + (skill.stun ?? 1),
          actionUsed: true,
        }));
        pushLog({
          side: "player",
          type: "stun",
          title: `${skill.name}`,
          desc: `Atordoa por ${skill.stun ?? 1}T`,
        });
        setMascotMood("happy");
        // FX: stun no oponente
        setStunFx((f) => ({ ...f, opponent: true }));
        setTimeout(() => setStunFx((f) => ({ ...f, opponent: false })), 900);
      } else {
  ...
  const onPlayerBenchClick = useCallback(
    (card, index) => {
    ...
    if (lastKOSide === "player") {
      ...
      if (hasAlive) setForcePromotionFor("player");
        ...
        pushLog({
          ...
          type: "damage",
          title: "Derrota",
          desc: "Você perdeu todos os Encantados.",
        });
      }
      ...
      if (idx >= 0) switchActive("opponent", idx);
        ...
        pushLog({
          ...
          type: "damage",
          title: "Vitória",
          desc: "O oponente não tem Encantados vivos.",
        });
      }
    }
    setLastKOSide(null);
  ...
  return (
    <div className="h-screen bg-gradient-to-br from-[#09131d] via-[#0c1f31] to-[#09131d] text-white overflow-hidden relative">
      {/* floatUp animation moved to globals.css */}
      ...
      >
          ...
          <div className="w-full max-w-6xl flex flex-col items-center">
            <div className="w-full flex justify-center mb-3">
              <BenchZone
                ...
                onCardContextMenu={(card) =>
                  setDetailCard(card?.foiRevelada ? card : null)
                }
              />
            </div>
            <div className="w-full flex justify-center">
              <div className="min-h-44 min-w-[20rem] flex items-center justify-center">
                <div
                  className={`relative ${
                    hasFieldBuff(activeCards.opponent)
                      ? "ring-2 ring-offset-2 ring-offset-black/30 ring-red-400/80 rounded-xl"
                      : ""
                  }`}
                >
                  <ActiveZone
                    ...
                    position="opponent"
                    onCardClick={() =>
                      console.log("Opponent active card clicked")
                    }
                    onCardContextMenu={(card) => setDetailCard(card)}
                  />
                  {/* FX oponente */}
                  {flashFx.opponent && (
                    <div className="absolute inset-0 bg-white/30 rounded-xl animate-pulse pointer-events-none" />
                  )}
                  {stunFx.opponent && (
                  ...
                  {damageFx
                    ...
                    .map((fx) => (
                      <div
                        key={fx.id}
                        className="absolute -top-4 left-1/2 -translate-x-1/2 text-red-300 font-extrabold text-lg animate-[floatUp_0.7s_ease-out_forwards] drop-shadow-[0_2px_6px_rgba(0,0,0,0.9)]"
                      >
                        -{fx.amount}
                      </div>
                    ))}
        ...
        <div className="row-start-3 flex flex-col items-center justify-start px-3 md:px-6">
          <div className="w-full max-w-6xl flex flex-col items-center">
            <div className="w-full flex justify-center relative z-[60]">
              <div className="min-h-48 min-w-[22rem] flex items-center justify-center">
                <div className="relative">
                  <div
                    className={`${
                      hasFieldBuff(activeCards.player)
                        ? "ring-2 ring-offset-2 ring-offset-black/30 ring-emerald-400/80 rounded-xl"
                        : ""
                    }`}
                  >
                    <ActiveZone
                  ...
                  {activeCards.player &&
                    ...
                    showOrbs && (
                      <SkillOrbs
                        ...
                        switchDisabled={
                          gameState.actionUsed || (gameState.playerStun ?? 0) > 0
                        }
                      />
                    )}
                </div>
            ...
            {awaitingSwitch && (
              <div className="mt-2 text-xs text-emerald-300 bg-emerald-800/30 border border-emerald-600 rounded-full px-3 py-1 shadow">
                Selecione um Encantado do banco para trocar
              </div>
            )}
          </div>
        </div>
        ...
        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[100vw] px-2">
          <div className="relative z-30 max-w-[100vw] overflow-x-hidden">
            <PlayerHand
              cards={playerBench.filter(Boolean).slice(0, 5)}
              selectedCard={null}
              onCardSelect={() => {}}
              onCardPlay={() => {}}
...
function DebugPanel({
  ...
  const healCard = (side, slot = "active") => {
    if (slot === "active") {
      setActiveCards((prev) => ({
        ...prev,
        [side]: prev[side]
          ? { ...prev[side], vida: prev[side].vidaMaxima || 100, isDead: false }
          : prev[side],
      }));
    }
  };
  ...
  const reviveAllCards = (side) => {
    if (side === "player") {
      setPlayerBench((prev) =>
        prev.map((card) =>
          card && card.isDead
            ? { ...card, vida: card.vidaMaxima || 100, isDead: false }
            : card
        )
      );
    } else {
  ...
  const cardOptions = bancoDeCartas
    .slice(0, 10)
    .map((c) => ({ id: c.id, name: c.nome }));
  ...
  return (
    <div className="fixed top-4 left-4 right-4 z-50 bg-black/90 backdrop-blur-md border border-neutral-600 rounded-xl p-4 max-h-[80vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <div className="text-xs text-neutral-300 mb-2">
              Player: {activeCards.player?.vida || 0}/
              {activeCards.player?.vidaMaxima || 100} | Oponente:{" "}
              {activeCards.opponent?.vida || 0}/
              {activeCards.opponent?.vidaMaxima || 100}
            </div>
            <div className="space-y-2">
              <div className="flex gap-1">
                <button
                  onClick={() => {
                    console.log(
                      "Player +10 clicked, current card health:",
                      activeCards.player?.vida
                    );
                    setActiveCards((prev) => ({
                      ...prev,
                      player: prev.player
                        ? {
                            ...prev.player,
                            vida: Math.min(
                              prev.player.vidaMaxima || 100,
                              (prev.player.vida || 0) + 10
                            ),
                            isDead: false,
                          }
                        : prev.player,
              ...
              <div className="flex gap-1">
                <button
                  onClick={() => {
                    console.log(
                      "Opponent +10 clicked, current card health:",
                      activeCards.opponent?.vida
                    );
                    setActiveCards((prev) => ({
                      ...prev,
                      opponent: prev.opponent
                        ? {
                            ...prev.opponent,
                            vida: Math.min(
                              prev.opponent.vidaMaxima || 100,
                              (prev.opponent.vida || 0) + 10
                            ),
                            isDead: false,
                          }
                        : prev.opponent,
      ...
      {activeTab === "cards" && (
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <div className="space-y-2">
              <div className="text-xs text-neutral-300">
                Ativo: {activeCards.player?.nome || "Nenhum"} (T:{" "}
                {activeCards.player?.onFieldTurns || 0})
              </div>
              <div className="flex gap-1">
                <button
                  onClick={() => healCard("player")}
                  className="flex-1 px-2 py-1 bg-green-600/80 text-white text-xs rounded"
                >
                  Curar
                </button>
                <button
                  onClick={() => killCard("player")}
                  className="flex-1 px-2 py-1 bg-red-600/80 text-white text-xs rounded"
                >
                  Matar
                </button>
              </div>
              <button
                onClick={() => restorePP("player")}
                className="w-full px-2 py-1 bg-blue-600/80 text-white text-xs rounded"
              >
                Restaurar PP
              </button>
              <button
                onClick={() => reviveAllCards("player")}
                className="w-full px-2 py-1 bg-purple-600/80 text-white text-xs rounded"
              >
                Reviver Mortas
              </button>
              <div className="flex gap-1">
                <button
                  onClick={() => addTurns("player", 1)}
                  className="flex-1 px-2 py-1 bg-yellow-600/80 text-white text-xs rounded"
                >
                  +1T
                </button>
                <button
                  onClick={() => addTurns("player", -1)}
                  className="flex-1 px-2 py-1 bg-orange-600/80 text-white text-xs rounded"
                >
                  -1T
                </button>
              </div>
          ...
          <div>
            ...
            <div className="space-y-2">
              <div className="text-xs text-neutral-300">
                Ativo: {activeCards.opponent?.nome || "Nenhum"} (T:{" "}
                {activeCards.opponent?.onFieldTurns || 0})
              </div>
              <div className="flex gap-1">
                <button
                  onClick={() => healCard("opponent")}
                  className="flex-1 px-2 py-1 bg-green-600/80 text-white text-xs rounded"
                >
                  Curar
                </button>
                <button
                  onClick={() => killCard("opponent")}
                  className="flex-1 px-2 py-1 bg-red-600/80 text-white text-xs rounded"
                >
                  Matar
                </button>
              ...
              <button
                onClick={() => restorePP("opponent")}
                className="w-full px-2 py-1 bg-blue-600/80 text-white text-xs rounded"
              >
                Restaurar PP
              </button>
              <button
                onClick={() => reviveAllCards("opponent")}
                className="w-full px-2 py-1 bg-purple-600/80 text-white text-xs rounded"
              >
                Reviver Mortas
              </button>
      ...
      {activeTab === "field" && (
        <div>
          <h4 className="text-sm font-bold text-white mb-2">
            Campo Atual: {fields[currentField]?.name}
          </h4>
